...
class Account:
    def __init__(self, golden_key: str, user_agent: str | None = None,
                 requests_timeout: int | float = 20, proxy: Optional[dict] = None,
                 locale: Literal["ru", "en", "uk"] | None = None):
        
        if golden_key:
            cleaned_key = "".join(golden_key.split())
            self.golden_key = re.sub(r'[^a-zA-Z0-9]', '', cleaned_key)
        else:
            self.golden_key = ""
        ...
        # Инициализируем сессию для главного потока сразу
        self._get_session()

    def _get_session(self) -> tls_client.Session:
        """
        Возвращает сессию tls_client для текущего потока.
        Если сессии нет, создает новую.
        """
        if not hasattr(self._thread_local, "session"):
            self._thread_local.session = tls_client.Session(
                client_identifier="chrome_120",
                random_tls_extension_order=True
            )
            # Применяем прокси к новой сессии
            if self._proxy:
                self._thread_local.session.proxies = self._proxy
                
        return self._thread_local.session

    @property
    def session(self) -> tls_client.Session:
        """Свойство для обратной совместимости, возвращает локальную сессию потока."""
    ...
    def method(self, request_method: Literal["post", "get"], api_method: str, headers: dict, payload: Any,
        ...
        cookie_string = f"golden_key={self.golden_key}; cookie_prefs=1"
        if self.phpsessid and not exclude_phpsessid:
            cookie_string += f"; PHPSESSID={self.phpsessid}"
        ...
        data_arg = payload
        if isinstance(payload, MultipartEncoder):
            data_arg = payload.to_string()
            req_headers["Content-Type"] = payload.content_type
        ...
        for i in range(attempts):
            try:
                response = current_session.execute_request(
                ...
                logger.error(f"Ошибка tls_client: {e}")
                if i == attempts - 1:
                    fake_resp = requests.Response()
                    fake_resp.status_code = 0
                    fake_resp.url = link
        ...
        if not hasattr(response, 'cookies'):
            ...
            for cookie_name, cookie_val in current_session.cookies.items():
                response.cookies.set(cookie_name, cookie_val)
        ...
        if response.status_code == 403:
            ...
            if response.status_code == 400:
                logger.critical(f"!!! FUNPAY ВЕРНУЛ 400 BAD REQUEST !!! URL: {link}")
    ...
    def get(self, update_phpsessid: bool = True) -> Account:
        ...
        if not (username_div := parser.find("div", {"class": "user-link-name"})):
            if "Подтвердите, что вы не робот" in html_response or "recaptcha" in html_response:
                logger.critical("ОБНАРУЖЕНА КАПЧА! IP заблокирован или требует проверки.")
            
            r = requests.Response()
            r.status_code = response.status_code
            r._content = response.content
            r.url = "https://funpay.com/"
            raise exceptions.UnauthorizedError(r)
            
        self.username = username_div.text

        body_tag = parser.find("body")
        if not body_tag or not body_tag.get("data-app-data"):
            r = requests.Response()
            r.status_code = 0
            r._content = b"No app-data"
            r.url = "https://funpay.com/"
            raise exceptions.RequestFailedError(r)

        try:
            self.app_data = json.loads(body_tag.get("data-app-data"))
        except json.JSONDecodeError:
            r = requests.Response()
            r.status_code = 0
            r._content = b"JSON Decode Error"
            r.url = "https://funpay.com/"
            raise exceptions.RequestFailedError(r)

        self.__locale = self.app_data.get("locale")
        self.id = self.app_data.get("userId")
        self.csrf_token = self.app_data.get("csrf-token")
        if not self.id or not self.csrf_token:
            r = requests.Response()
            r.status_code = 403
            r._content = b"No ID or CSRF"
            r.url = "https://funpay.com/"
            raise exceptions.UnauthorizedError(r)

        if (logout_link_tag := parser.find("a", class_="menu-item-logout")):
            self._logout_link = logout_link_tag.get("href")

        active_sales_span = parser.find("span", {"class": "badge badge-trade"})
        self.active_sales = int(active_sales_span.text) if active_sales_span and active_sales_span.text.isdigit() else 0

        balance_span = parser.find("span", class_="badge badge-balance")
        if balance_span:
            try:
                balance_text, currency_text = balance_span.text.rsplit(" ", maxsplit=1)
                self.total_balance = int(balance_text.replace(" ", ""))
                self.currency = parse_currency(currency_text)
            except (ValueError, IndexError):
                self.total_balance = 0
        else:
            self.total_balance = 0

        active_purchases_span = parser.find("span", {"class": "badge badge-orders"})
        self.active_purchases = int(active_purchases_span.text) if active_purchases_span and active_purchases_span.text.isdigit() else 0

        if update_phpsessid or not self.phpsessid:
             self.phpsessid = self.session.cookies.get("PHPSESSID", self.phpsessid)
    ...
    def upload_image(self, image: str | IO[bytes], type_: Literal["chat", "offer"] = "chat") -> int:
        ...
        if response.status_code == 400:
            try:
                ...
                r.status_code = 400
                r._content = response.content
                r.url = f"file/add{type_.title()}Image"
                raise exceptions.ImageUploadError(r, message)
            except Exception:
                 r = requests.Response()
                 r.status_code = 400
                 r._content = response.content
                 r.url = f"file/add{type_.title()}Image"
                 raise exceptions.ImageUploadError(r, None)
    ...
    def send_message(self, chat_id: int | str, text: Optional[str] = None, chat_name: Optional[str] = None,
        ...
        if not (resp := json_response.get("response")):
             ...
             r._content = response.content
             r.url = "runner/"
             raise exceptions.MessageNotDeliveredError(r, None, chat_id)
        ...
        if (error_text := resp.get("error")) is not None:
            ...
            r._content = response.content
            r.url = "runner/"
            raise exceptions.MessageNotDeliveredError(r, error_text, chat_id)
    ...
    def send_review(self, order_id: str, text: str, rating: Literal[1, 2, 3, 4, 5] = 5) -> str:
        ...
        if response.status_code == 400:
            ...
            r._content = response.content
            r.url = "orders/review"
            raise exceptions.FeedbackEditingError(r, msg, order_id)
        elif response.status_code != 200:
             r = requests.Response()
             r.status_code = response.status_code
             r._content = response.content
    ...
    def refund(self, order_id):
        ...
        if response.json().get("error"):
             ...
             r._content = response.content
             r.url = "orders/refund"
             raise exceptions.RefundError(r, response.json().get("msg"), order_id)
    ...
    def withdraw(self, currency: enums.Currency, wallet: enums.Wallet, amount: int | float, address: str) -> float:
        ...
        if json_response.get("error"):
            ...
            r.status_code = response.status_code
            r._content = response.content
            r.url = "withdraw/withdraw"
            raise exceptions.WithdrawError(r, error_message)
    ...
    def raise_lots(self, category_id: int, subcategories: Optional[list[int | types.SubCategory]] = None,
        ...
        if not json_response.get("error") and not json_response.get("url"):
            ...
            raise exceptions.RaiseError(r, category, json_response.get("url"), 7200)
            ...
            raise exceptions.RaiseError(r, category, json_response.get("msg"), wait_time)
        else:
            raise exceptions.RaiseError(r, category, json_response.get("msg"), None)
    ...
    def get_lot_fields(self, lot_id: int) -> types.LotFields:
        ...
        if error_message:
             ...
             r.status_code = 200
             r._content = response.content
             r.url = f"lots/offerEdit?offer={lot_id}"
             raise exceptions.LotParsingError(r, error_message.text, lot_id)
        result = {}
    ...
    def save_lot(self, lot_fields: types.LotFields):
        ...
        if (errors := json_response.get("errors")) or json_response.get("error"):
            ...
            r.status_code = 200
            r._content = response.content
            r.url = "lots/offerSave"
            raise exceptions.LotSavingError(r, json_response.get("error"), lot_fields.lot_id, errors_dict)
